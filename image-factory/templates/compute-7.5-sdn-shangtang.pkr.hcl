locals {
  ...
  #iso_checksum  = "5822eb7b6c400f13bb8d0cc9195bf378"
  #iso_url       = "http://10.177.1.129/autoinstall/tarball/centos-7.5-x86_64.tgz"
  os_version    = 7.5
  timestamp     = formatdate("YYYYMMDDhhmmss", timestamp())
  image_id      = try(var.git_commit_id, local.timestamp)
  img_name      = "${local.os_type}-${local.os_version}-shangtang-${local.node_type}-sdn-${local.image_id}.qcow2"
}
...
build {
  ...
  provisioner "shell" {
    scripts = [
      #"scripts/install.d/110-copy.bash",
