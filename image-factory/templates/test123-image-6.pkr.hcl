locals {
  ...
  iso_url       = "http://luna.galaxy.ksyun.com/images/baremetal/base/CentOS-6.5-base-latest.qcow2"
  os_version    = 6.5
  timestamp     = formatdate("YYYYMMDDhhmmss", timestamp())
  image_id      = try(var.git_commit_id, local.timestamp)
  img_name      = "${local.os_type}-${local.os_version}-${local.node_type}123-${local.image_id}.qcow2"
}
