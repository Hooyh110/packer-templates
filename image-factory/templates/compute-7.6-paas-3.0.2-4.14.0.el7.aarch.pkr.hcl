...
source "qemu" "base" {
  ...
  boot_command = [
        "<wait>",                                      // 等待虚拟机启动
        "c",                                           // 模拟按下 "c" 键
        "<tab>   linux /images/pxeboot/vmlinuz inst.stage2=hd:LABEL=CentOS\\x207\\x20aarch64 text biosdevname=0 net.ifnames=0 ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/${local.anwserfile}<enter>",
