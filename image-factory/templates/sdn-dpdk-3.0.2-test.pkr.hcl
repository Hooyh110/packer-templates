locals {
  ...
  iso_checksum  = "1dc4bd2c0905fd3079780bc2e4cb1fb8"
  iso_url       = "http://10.91.128.61/images/baremetal/3.0.2/sdn-dpdk/CentOS-6.5-sdn-dpdk-3.10.0-123.6.1.el6.ksyun.x86_64.qcow2"
  os_version    = 6.5
...
build {
  ...
  provisioner "file" {
    destination = "/etc/yum.repos.d/centos-6.repo"
    source      = "http/centos-6.repo"
  }
  provisioner "file" {
    destination = "/etc/"
...
//   }
   ...
   provisioner "shell" {
     scripts = [
       ...
       "scripts/install.d/90-raid-drive-sas-dell-3.0.2.bash",
