locals {
  ...
  iso_url       = "http://10.91.128.61/images/iso/CentOS-7-x86_64-Minimal-1804.iso"
  os_version    = 7.5
  timestamp     = formatdate("YYYYMMDDhhmmss", timestamp())
  image_id      = try(var.git_commit_id, local.timestamp)
  img_name      = "${local.os_type}-${local.os_version}-3.0.2-msdos-${local.image_id}.qcow2"
}
...
build {
  ...
  provisioner "shell" {
    scripts = [
      "scripts/install.d/90-network-drive-i40e-3.0.2.bash",
      "scripts/install.d/90-raid-drive-mpt3sas-3.0.2.bash",
      "scripts/install.d/90-raid-drive-megaraid_sas_7.5.bash",
