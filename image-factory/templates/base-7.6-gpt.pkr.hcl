locals {
  ...
  anwserfile    = "anwserfiles/ks_centos_7.6_shangtang_gpt.cfg"
  iso_checksum  = "file:http://luna.galaxy.ksyun.com/images/iso/sha1sum.txt"
  iso_url       = "http://luna.galaxy.ksyun.com/images/iso/CentOS-7.6-x86_64-Minimal-1810.iso"
  os_version    = 7.6
  timestamp     = formatdate("YYYYMMDDhhmmss", timestamp())
  image_id      = try(var.git_commit_id, local.timestamp)
  img_name      = "${local.os_type}-${local.os_version}-shangtang-gpt-${local.node_type}-${local.image_id}.qcow2"
}
