locals {
  ...
  iso_url       = "http://luna.galaxy.ksyun.com/images/baremetal/base/CentOS-7.3-base-8358p-20220916-new.qcow2"
  os_version    = 7.3
  timestamp     = formatdate("YYYYMMDDhhmmss", timestamp())
  image_id      = try(var.git_commit_id, local.timestamp)
  img_name      = "${local.os_type}-${local.os_version}-${local.node_type}-8358p.qcow2"
}
